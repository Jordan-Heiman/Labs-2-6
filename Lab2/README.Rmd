...
# Ensure that knitr is installed and loaded
require(knitr)

# Set global knit options so that code is echo'd
knitr::opts_chunk$set(echo = TRUE)

# Change r reposoitory, not sure I know why this is needed ///JLH 1-31-23
r <- getOption("repos")
...
```{r list packages, eval = TRUE, results = 'hide'}
...
```{r load packages, eval = TRUE, message = FALSE, results = 'hide'}
...
As a preliminary step, let's plot some of the wolfyht using the base plot function, and using some of the structure of the wolfyht object. Let's remind ourselves of the structure of the sf object wolfyht. 
```{r plot wolf data, message = FALSE}
...
For today's lab we will use elevation and distance to human access rasters we created before the lab and re-load them from our working directory. 
```{r read in elevation/human access}
...
```{r interactive rasters, warning = FALSE, eval = FALSE}
# Not working here
# library(mapview)
...
```{r rasterize human access, eval = FALSE}
...
```{r data formatting, warning=FALSE}
...
of the first animal in rd1_ud as a RASTER file of 1= home range, 0 = not home range. 
```{r store UD volume, warning= FALSE}
...
Store the volume under the UD (as computed by getvolumeUD) of the first animal in bv1_ud. 
```{r}
...
Store the volume under the UD (as computed by getvolumeUD) of the first pack in pack1_ud 
```{r}
...
```{r}
multhist(list(wolfused$WOLF_W[wolfused$pack == "Bow Valley"],
              wolfused$WOLF_W[wolfused$pack == "Red Deer"]),
         freq = TRUE,
         main = "Wolf HSI",
         legend.text = c("Bow Valley", "Red Deer"), 
         args.legend = list(bty = "n"))
